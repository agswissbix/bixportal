...
export default function TestConnection() {
  ...
  useEffect(() => {
    ...
    extractCookies();
    // Aggiorna i cookie dopo ogni richiesta
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-white rounded-md shadow-sm border border-gray-200">
              ...
              <button 
                ...
                disabled={loading}
                className="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-200 disabled:bg-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
              >
            ...
            <div className="p-4 bg-white rounded-md shadow-sm border border-gray-200">
              ...
              <button 
                ...
                disabled={loading}
                className="w-full px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition duration-200 disabled:bg-green-300 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
              >
                Ottieni CSRF
              </button>
            ...
            <div className="p-4 bg-white rounded-md shadow-sm border border-gray-200">
              ...
              <button 
                ...
                disabled={loading}
                className="w-full px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition duration-200 disabled:bg-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50"
              >
                Invia POST
              </button>
            </div>
        ...
        <div className="mb-8 p-4 bg-yellow-50 rounded-md">
          <h2 className="text-xl font-semibold text-yellow-800 mb-3">Cookie della Sessione</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full border border-yellow-200 rounded-md">
              <thead className="bg-yellow-100">
                <tr>
                  <th className="px-4 py-2 text-left text-yellow-800 border-b">Nome Cookie</th>
                  <th className="px-4 py-2 text-left text-yellow-800 border-b">Valore</th>
                </tr>
              </thead>
              <tbody>
                {Object.keys(cookies).length > 0 ? (
                  Object.entries(cookies).map(([key, value], index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-yellow-50'}>
                      <td className="px-4 py-2 border-b font-medium">
                        {key === 'csrftoken' || key === 'sessionid' 
                          ? <span className="font-bold text-yellow-700">{key}</span>
                          : key
                        }
                      </td>
                      <td className="px-4 py-2 border-b font-mono break-all">
                        {key === 'csrftoken' || key === 'sessionid' 
                  ...
                  <tr>
                    <td colSpan={2} className="px-4 py-2 text-center text-gray-500">Nessun cookie disponibile</td>
                  </tr>
                )}
              </tbody>
            </table>
