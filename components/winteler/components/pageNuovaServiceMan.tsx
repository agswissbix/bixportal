...
export default function PageNuovaServiceMan({ propExampleValue }: PropsInterface) {
    //DATI
    ...
    const handleChange = (path: string, value: any) => {
        setResponseData(prev => {
            ...
            if (path === 'data') {
                const existingDate = prev.serviceMan.data ? new Date(prev.serviceMan.data) : new Date();
    ...
    const rimuoviFoto = (indexToRemove: number) => {
        ...
        setResponseData(prev => ({
            ...
            serviceMan: {
                ...prev.serviceMan,
                foto: prev.serviceMan.foto.filter((_, index) => index !== indexToRemove)
            }
        }));
    ...
    const handleSubmit = (event: React.FormEvent) => {
        saveServiceMan(
            responseData.serviceMan
        );
    };

    async function saveServiceMan({nome, cognome, telefono, email, targa, telaio, modello, data, utente}: ServiceMan) {
        try {
                                ...
                                <div className='p-4'>
                                    ...
                                    <div className="mb-8 mt-8">
                                        <FloatingLabelInput
                                            ...
                                            label="Data"
                                            type='date'
                                            value={formatDateForInput(response.serviceMan.data)}
                                    ...
                                    <div className="mb-8 mt-8">
                                        <FloatingLabelInput
                                            ...
                                            label="Ora "
                                            type='time'
                                            value={formatTimeForInput(response.serviceMan.data)}
                                            onChange={(e) => handleChange('data', e.target.value)}
                                        />
                                    </div>
                                    ...
                                    <GeneralButton
                                        text={response.serviceMan.foto.length > 0 ? 'Aggiungi Foto' : 'Carica Foto'} 
                                        className='mb-4'
                                        action={caricaFoto}
                                        type="button" 
                                    />
