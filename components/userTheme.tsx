...
import { Palette } from 'lucide-react'; // Icone per migliorare l'interfaccia
  ...
  return (
    <GenericComponent response={responseData} loading={loading} error={error}>
      {(response: ResponseInterface) => (
        <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl mx-auto my-10">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
            Scegli il tuo tema preferito per personalizzare l'aspetto dell'applicazione.
          </p>
              ...
              <div key={themeName} className="flex flex-col items-center">
                ...
                >
                  <div className="flex flex-col space-y-1">
                    <div
                      className="w-12 h-4 rounded-t-sm"
                      style={{ backgroundColor: themeConfig[themeName].primary }}
                      title={`Primario: ${themeConfig[themeName].primary}`}
                    ></div>
                    <div
                      className="w-12 h-4"
                      style={{ backgroundColor: themeConfig[themeName].secondary }}
                      title={`Secondario: ${themeConfig[themeName].secondary}`}
                    ></div>
                    <div
                      className="w-12 h-4 rounded-b-sm"
                      style={{ backgroundColor: themeConfig[themeName].tertiary }}
                      title={`Terziario: ${themeConfig[themeName].tertiary}`}
                    ></div>
                  </div>
                </button>
