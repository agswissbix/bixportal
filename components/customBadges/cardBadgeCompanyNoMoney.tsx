...
import { Crown, ShieldX, OctagonAlert, BadgeCheck, BadgeMinus, TrendingUp, Cog, DollarSign, Clock, Target, Phone, Database, Lock, ChevronDown, CheckCircle, Server, Box, Globe, Shield, ExternalLink, Info, ChevronUp, Mail, MapPin } from 'lucide-react';
...
export default function CardBadgeCompany({ tableid, recordid }: PropsInterface) {
  ...
  return (
    <GenericComponent response={responseData} error={error} title="Company Badge">
        ...
        {loading ? (
          ...
          <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            >
              <div className="p-6">
                {/* Top metrics cards */}
                {/* Top metrics and info */}
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                  {/* Contact Info Card */}
                  <div className="flex-1 bg-[#f8fafc] border border-gray-100/50 rounded-xl shadow-sm p-5 flex flex-col justify-center">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Recapiti Aziendali</h3>
                    <div className="space-y-3 text-sm text-gray-700">
                      {response.badgeItems.company_email && (
                        <div className="flex items-center gap-3">
                          <Mail className="w-4 h-4 text-accent flex-shrink-0" />
                          <a href={`mailto:${response.badgeItems.company_email}`} className="hover:text-accent transition-colors truncate">
                            {response.badgeItems.company_email}
                          </a>
                        </div>
                      )}
                      {response.badgeItems.company_phone && (
                        <div className="flex items-center gap-3">
                          ...
                          <a href={`tel:${response.badgeItems.company_phone}`} className="hover:text-accent transition-colors">
                            {response.badgeItems.company_phone}
                          </a>
                        </div>
                      )}
                      {response.badgeItems.company_address && (
                        <div className="flex items-start gap-3">
                          ...
                          <span className="leading-snug">{response.badgeItems.company_address}</span>
                        </div>
                      )}
                      {!response.badgeItems.company_email && !response.badgeItems.company_phone && !response.badgeItems.company_address && (
                        <span className="text-gray-400 italic">Nessun recapito inserito.</span>
                      )}
                    </div>
                  </div>

                  {/* Totale Lavori Metric */}
