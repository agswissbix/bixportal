...
// DATI DI DEFAULT PER LA RISPOSTA DAL BACKEND
const responseDataDEFAULT: ResponseInterface = {
    menuItems: {}
};

// DATI PER LO SVILUPPO IN LOCALE
...
export default function ExampleComponentWithData({ selectedMenu }: PropsInterface) {
    // PROPS PER LO SVILUPPO
    const devPropExampleValue = isDev ? "Valore di test" : selectedMenu;
    ...
    // Stato per la risposta dal backend (o mockata)
    const [responseData, setResponseData] = useState<ResponseInterface>(
        isDev ? responseDataDEV : responseDataDEFAULT
    );

    // Se non siamo in sviluppo, chiama il backend
    const { response, loading, error } = isDev
        ? { response: responseDataDEV, loading: false, error: null } // Evita la chiamata API
        : useApi<ResponseInterface>({ 
            apiRoute: 'examplepost', 
            payloadExampleValue: devPropExampleValue 
        });

    // Aggiorna i dati solo se non siamo in sviluppo
    useEffect(() => {
        if (!isDev && response) {
